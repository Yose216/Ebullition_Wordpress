...
function Add_Edit_Product() {
	...
	$Item_Sale_Price = sanitize_text_field($_POST['Item_Sale_Price']);
	if ($_POST['Item_Sale_Mode']) {$Item_Sale_Mode = "Yes";}
	...
	if (!isset($error) or $error == __('No file was uploaded.', 'UPCP')) {
			...
			/* Pass the data to the appropriate function in Update_Admin_Databases.php to edit the product */
					...
					$user_update = Edit_UPCP_Product($Item_ID, $Item_Name, $Item_Slug, $Item_Photo_URL, $Item_Description, $Item_Price, $Item_Sale_Price, $Item_Sale_Mode, $Item_SEO_Description, $Item_Link, $Item_Display_Status, $Category_ID, $Global_Item_ID, $Item_Special_Attr, $SubCategory_ID, $Tags, $Related_Products, $Next_Previous);
			}
...
function Add_Products_From_Spreadsheet() {
		...
		if(!preg_match("/\.(xls.?)$/", $_FILES['Products_Spreadsheet']['name']) and !preg_match("/\.(csv.?)$/", $_FILES['Products_Spreadsheet']['name'])) {
			$error = __('File must be .csv, .xls, .xlsx', 'UPCP');
		}
...
function Add_Edit_Category() {
		...
		$Category_Image = stripslashes_deep($_POST['Category_Image']);
		$Category_ID = $_POST['Category_ID'];
		...
		if (!isset($error)) {
				...
				/* Pass the data to the appropriate function in Update_Admin_Databases.php to edit the category */
						...
						$user_update = Edit_UPCP_Category($Category_ID, $Category_Name, $Category_Description, $Category_Image);
				}
				$user_update = array("Message_Type" => "Update", "Message" => $user_update);
				return $user_update;
		}
...
function Add_Edit_SubCategory() {
		...
		if (!isset($error)) {
				...
				/* Pass the data to the appropriate function in Update_Admin_Databases.php to edit the sub-category */
						...
						$user_update = Edit_UPCP_SubCategory($SubCategory_ID, $SubCategory_Name, $Category_ID, $SubCategory_Description, $SubCategory_Image);
				}
				$user_update = array("Message_Type" => "Update", "Message" => $user_update);
				return $user_update;
		}
...
function Add_Edit_Custom_Field() {
		...
		if (!isset($error)) {
				...
				if ($_POST['action'] == "Add_Custom_Field") {
					  $user_update = Add_UPCP_Custom_Field($Field_Name, $Field_Slug, $Field_Type, $Field_Description, $Field_Values, $Field_Displays, $Field_Searchable, $Field_Display_Tabbed, $Field_Control_Type);
				}
				/* Pass the data to the appropriate function in Update_Admin_Databases.php to edit the custom field */
				else {
						$user_update = Edit_UPCP_Custom_Field($Field_ID, $Field_Name, $Field_Slug, $Field_Type, $Field_Description, $Field_Values, $Field_Displays, $Field_Searchable, $Field_Display_Tabbed, $Field_Control_Type);
				}
				$user_update = array("Message_Type" => "Update", "Message" => $user_update);
				return $user_update;
		}
